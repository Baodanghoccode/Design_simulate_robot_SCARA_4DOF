...
class SerialManager:
    """Quản lý kết nối Serial với Arduino trong thread riêng."""
    def __init__(self):
        self.ser        = None
    ...
    def send(self, t1, t2, d3, t4):
        if not self.connected or not self.ser:
            return False, "Chưa kết nối"
        cmd = json.dumps({"t1": round(t1,2), "t2": round(t2,2),
                          "d3": round(d3,2), "t4": round(t4,2)}) + "\n"
        try:
...
class App:
    ...
    def _build_serial_bar(self):
        """Thanh điều khiển Serial ở dưới cùng cửa sổ."""
        bar = tk.Frame(self.root, bg=TH["bg2"], height=44)
        bar.pack(fill="x", side="bottom")
        bar.pack_propagate(False)
        tk.Frame(bar, bg=TH["hi"], width=4).pack(side="left", fill="y")

        inner = tk.Frame(bar, bg=TH["bg2"])
        inner.pack(side="left", fill="both", expand=True, padx=8, pady=5)

        
        tk.Label(inner, text="🤖 ARDUINO", font=self.F["badge"],
                 bg=TH["bg2"], fg=TH["purple"]).pack(side="left", padx=(0,8))

        
        tk.Label(inner, text="COM:", font=self.F["badge"],
                 bg=TH["bg2"], fg=TH["text3"]).pack(side="left")

        self._com_var = tk.StringVar()
        ports = SerialManager.list_ports()
        self._com_cb = tk.OptionMenu(inner, self._com_var,
                                     *(ports if ports else ["—"]))
        self._com_cb.config(font=self.F["tiny"], bg=TH["input"], fg=TH["text"],
                            bd=0, highlightthickness=0, activebackground=TH["card2"],
                            activeforeground=TH["text"], padx=4)
        self._com_cb["menu"].config(bg=TH["card2"], fg=TH["text"],
                                    font=self.F["tiny"])
        if ports:
            self._com_var.set(ports[0])
        self._com_cb.pack(side="left", padx=(2,4))

        def _btn(txt, fg, cmd):
            return tk.Button(inner, text=txt, font=self.F["badge"],
                             bg=TH["card2"], fg=fg, bd=0, padx=10, pady=3,
                             cursor="hand2", command=cmd,
                             activebackground=TH["input"],
                             activeforeground=fg)

        _btn("⟳", TH["cyan"], self._serial_refresh_ports).pack(side="left", padx=2)
        self._btn_conn = _btn("⏻ Kết nối", TH["green"], self._serial_toggle)
        self._btn_conn.pack(side="left", padx=4)

        # ─ Gửi tay ─
        _btn("▶ Gửi", TH["cyan"], self._serial_send_now).pack(side="left", padx=2)

        # ─ Auto send ─
        tk.Checkbutton(inner, text="Auto", variable=self.auto_send,
                       font=self.F["badge"], bg=TH["bg2"],
                       fg=TH["text2"], selectcolor=TH["input"],
                       activebackground=TH["bg2"],
                       activeforeground=TH["text"]).pack(side="left", padx=6)

        # ─ Baud ─
        tk.Label(inner, text="Baud:", font=self.F["tiny"],
                 bg=TH["bg2"], fg=TH["text3"]).pack(side="left")
        self._baud_var = tk.StringVar(value="115200")
        baud_om = tk.OptionMenu(inner, self._baud_var,
                                "9600", "57600", "115200", "250000")
        baud_om.config(font=self.F["tiny"], bg=TH["input"], fg=TH["text"],
                       bd=0, highlightthickness=0, activebackground=TH["card2"],
                       activeforeground=TH["text"], padx=2)
        baud_om["menu"].config(bg=TH["card2"], fg=TH["text"], font=self.F["tiny"])
        baud_om.pack(side="left", padx=(2,8))

        # ─ Status ─
        self._ser_status = tk.Label(inner, text="● Chưa kết nối",
                                    font=self.F["badge"], bg=TH["bg2"],
                                    fg=TH["rose"])
        self._ser_status.pack(side="left", padx=8)

        # ─ Response ─
        self._ser_resp = tk.Label(inner, text="—", font=self.F["tiny"],
                                  bg=TH["bg2"], fg=TH["text3"])
        self._ser_resp.pack(side="left", padx=4)

        self._serial_poll()

    def _serial_refresh_ports(self):
        ports = SerialManager.list_ports()
        menu = self._com_cb["menu"]
        menu.delete(0, "end")
        for p in (ports if ports else ["—"]):
            menu.add_command(label=p, command=lambda v=p: self._com_var.set(v))
        if ports:
            self._com_var.set(ports[0])
    ...
    def _serial_toggle(self):
        if self.serial_mgr.connected:
            ...
            self._ser_status.config(text="● Đã ngắt", fg=TH["amber"])
            self._btn_conn.config(text="⏻ Kết nối", fg=TH["green"])
        else:
            port = self._com_var.get()
            if not port or port == "—":
                self._ser_status.config(text="⚠ Chọn cổng COM!", fg=TH["amber"])
                return
            self._ser_status.config(text="⋯ Đang kết nối...", fg=TH["cyan"])
            self.root.update_idletasks()
            baud = int(self._baud_var.get())
            ok, msg = self.serial_mgr.connect(port, baud)
            if ok:
                self._ser_status.config(text=f"● {port} ✓", fg=TH["green"])
                self._btn_conn.config(text="✕ Ngắt", fg=TH["rose"])
            else:
                self._ser_status.config(text=f"✕ {msg[:35]}", fg=TH["rose"])

    def _serial_send_now(self):
        if not self.serial_mgr.connected:
            self._ser_status.config(text="⚠ Chưa kết nối!", fg=TH["amber"])
            return
        ok, msg = self.serial_mgr.send(
            self.t1.get(), self.t2.get(),
            self.d3.get(), self.t4.get())
        if not ok:
            self._ser_status.config(text=f"✕ {msg[:35]}", fg=TH["rose"])
    ...
    def _serial_poll(self):
        """Poll phản hồi từ Arduino mỗi 50ms."""
        try:
            while not self.serial_mgr.rx_queue.empty():
                ...
                try:
                    ...
                    st = data.get("status", "")
                    if st == "ok":
                        self._ser_resp.config(
                            text=(f"t1={data.get('t1',0):.1f}°  "
                                  f"t2={data.get('t2',0):.1f}°  "
                                  f"d3={data.get('d3',0):.1f}mm  "
                                  f"t4={data.get('t4',0):.1f}°"),
                            fg=TH["teal"])
                    elif st == "error":
                        self._ser_resp.config(
                            text=f"ERR: {data.get('msg','')}",
                            fg=TH["rose"])
                    elif st == "ready":
                        self._ser_resp.config(
                            text=data.get("msg", "Ready"),
                            fg=TH["green"])
                except json.JSONDecodeError:
                    self._ser_resp.config(text=line[:60], fg=TH["amber"])
        except Exception:
            pass
        self.root.after(50, self._serial_poll)
